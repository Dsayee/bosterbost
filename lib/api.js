...
export const getAdminData = () => request(`/api/admin?ts=${Date.now()}`);

export const getAdminStats = () => request(`/api/admin/stats?ts=${Date.now()}`);

export const getBackendStatus = () => request(`/api/backend/status?ts=${Date.now()}`);
