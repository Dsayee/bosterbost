...
export const getUserById = async (userId) => {
  ...
  const user = isMysqlEnabled
    ? (await mysqlRows("SELECT * FROM users WHERE id = ? LIMIT 1", [userId]))[0]
    : isCloudEnabled
      ? (await cloudRequest(`users?id=eq.${encodeURIComponent(userId)}&select=*&limit=1`))[0]
      : isCloudflareD1Enabled
        ? (await d1Query("SELECT * FROM users WHERE id = ? LIMIT 1", [userId]))[0]
...
export const updateOrderStatus = async (orderId, status) => {
  ...
  const refundDescription = (order) => `Refund for rejected order ${order.order_code || order.id}`;
  const refundTransactionId = (order) => `refund-${order.id}`;
  const shouldRefund = (order) => status === "Rejected" && order && order.status !== "Rejected";

  if (isMysqlEnabled) {
    return withMysqlConnection(async (connection) => {
      await connection.beginTransaction();
      try {
        const [orders] = await connection.execute("SELECT * FROM orders WHERE id = ? OR order_code = ? LIMIT 1 FOR UPDATE", [orderId, orderId]);
        ...
        if (shouldRefund(order)) {
          await connection.execute("UPDATE users SET wallet = wallet + ? WHERE id = ?", [order.cost, order.user_id]);
          await connection.execute(
            `INSERT INTO wallet_transactions (id, user_id, type, amount_rwf, original_amount, original_currency, description, created_at)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
            [refundTransactionId(order), order.user_id, "refund", order.cost, order.cost, "RWF", refundDescription(order), updatedAt]
          );
        }
        await connection.execute("UPDATE orders SET status = ?, updated_at = ? WHERE id = ?", [status, updatedAt, order.id]);
        await connection.commit();
        return { ...order, status, updated_at: updatedAt };
      } catch (error) {
        await connection.rollback();
        throw error;
      }
  ...
  if (isCloudEnabled) {
    const [existingOrder] = await cloudRequest(`orders?or=(id.eq.${encodeURIComponent(orderId)},order_code.eq.${encodeURIComponent(orderId)})&select=*&limit=1`);
    if (!existingOrder) return null;
    if (shouldRefund(existingOrder)) {
      const [user] = await cloudRequest(`users?id=eq.${encodeURIComponent(existingOrder.user_id)}&select=wallet&limit=1`);
      await cloudRequest(`users?id=eq.${encodeURIComponent(existingOrder.user_id)}`, {
        method: "PATCH",
        body: JSON.stringify({ wallet: Number(user?.wallet || 0) + Number(existingOrder.cost || 0) }),
      });
      await cloudRequest("wallet_transactions", {
        method: "POST",
        body: JSON.stringify({
          id: refundTransactionId(existingOrder),
          user_id: existingOrder.user_id,
          type: "refund",
          amount_rwf: Number(existingOrder.cost || 0),
          original_amount: Number(existingOrder.cost || 0),
          original_currency: "RWF",
          description: refundDescription(existingOrder),
          created_at: updatedAt,
        }),
      });
    }
    const [order] = await cloudRequest(`orders?id=eq.${encodeURIComponent(existingOrder.id)}`, {
      method: "PATCH",
      headers: { Prefer: "return=representation" },
  ...
  if (isCloudflareD1Enabled) {
    const existingOrder = (await d1Query("SELECT * FROM orders WHERE id = ? OR order_code = ? LIMIT 1", [orderId, orderId]))[0];
    if (!existingOrder) return null;
    const batch = [];
    if (shouldRefund(existingOrder)) {
      batch.push(
        { sql: "UPDATE users SET wallet = wallet + ? WHERE id = ?", params: [existingOrder.cost, existingOrder.user_id] },
        {
          sql: `INSERT INTO wallet_transactions (id, user_id, type, amount_rwf, original_amount, original_currency, description, created_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
          params: [refundTransactionId(existingOrder), existingOrder.user_id, "refund", existingOrder.cost, existingOrder.cost, "RWF", refundDescription(existingOrder), updatedAt],
        }
      );
    }
    batch.push({ sql: "UPDATE orders SET status = ?, updated_at = ? WHERE id = ?", params: [status, updatedAt, existingOrder.id] });
    await d1Batch(batch);
    return { ...existingOrder, status, updated_at: updatedAt };
  }

  const db = getDb();
  db.exec("BEGIN TRANSACTION;");
  try {
    const existingOrder = db.prepare("SELECT * FROM orders WHERE id = ? OR order_code = ?").get(orderId, orderId);
    if (!existingOrder) {
      db.exec("ROLLBACK;");
      return null;
    }
    if (shouldRefund(existingOrder)) {
      db.prepare("UPDATE users SET wallet = wallet + ? WHERE id = ?").run(existingOrder.cost, existingOrder.user_id);
      db.prepare(
        `INSERT INTO wallet_transactions (id, user_id, type, amount_rwf, original_amount, original_currency, description, created_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
      ).run(refundTransactionId(existingOrder), existingOrder.user_id, "refund", existingOrder.cost, existingOrder.cost, "RWF", refundDescription(existingOrder), updatedAt);
    }
    db.prepare("UPDATE orders SET status = ?, updated_at = ? WHERE id = ?").run(status, updatedAt, existingOrder.id);
    db.exec("COMMIT;");
    return { ...existingOrder, status, updated_at: updatedAt };
  } catch (error) {
    db.exec("ROLLBACK;");
...
export const listPendingPaymentDeposits = async ({ userId = "", limit = 20 } = {}) => {
  const safeLimit = Math.max(1, Math.min(Number(limit) || 20, 50));
  const pendingStatuses = ["PAYMENT_PAGE_CREATED", "ACCEPTED", "PENDING", "SUBMITTED"];

  if (isMysqlEnabled) {
    const statusPlaceholders = pendingStatuses.map(() => "?").join(",");
    const params = userId ? [...pendingStatuses, userId, safeLimit] : [...pendingStatuses, safeLimit];
    const rows = await mysqlRows(
      `SELECT * FROM payment_deposits
       WHERE status IN (${statusPlaceholders}) ${userId ? "AND user_id = ?" : ""}
  ...
  if (isCloudEnabled) {
    const statusList = pendingStatuses.map((status) => `"${status}"`).join(",");
    const userFilter = userId ? `&user_id=eq.${encodeURIComponent(userId)}` : "";
    const deposits = await cloudRequest(
      `payment_deposits?status=in.(${encodeURIComponent(statusList)})${userFilter}&select=*&order=created_at.asc&limit=${safeLimit}`
    );
    return deposits.map(mapPaymentDeposit);
  }
...
export const completePaymentDeposit = async (providerDepositId, providerResponse = null) => {
  const now = new Date().toISOString();
  const response = providerResponse ? JSON.stringify(providerResponse) : null;
  const transactionId = `pawapay-${providerDepositId}`;
  ...
  try {
    ...
    if (deposit.status !== "COMPLETED") {
      ...
      db.prepare(
        `INSERT INTO wallet_transactions (id, user_id, type, amount_rwf, original_amount, original_currency, description, created_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
      ).run(transactionId, deposit.user_id, "deposit", deposit.amount_rwf, deposit.original_amount, deposit.original_currency, `PawaPay deposit ${providerDepositId}`, now);
    }
